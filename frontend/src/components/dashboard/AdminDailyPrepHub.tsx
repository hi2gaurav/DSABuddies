...
export const AdminDailyPrepHub: React.FC = () => {
  ...
  return (
    <Card className="p-0 overflow-hidden border-2 border-amber-400/40 dark:border-amber-500/30 shadow-xl bg-gradient-to-b from-amber-500/[0.03] via-white to-white dark:via-slate-800 dark:to-slate-850 relative">
      ...
      {refreshSuccess && (
        <div className="bg-emerald-500/15 border-b border-emerald-500/30 px-6 py-2.5 flex items-center justify-between text-xs text-emerald-800 dark:text-emerald-300 font-bold transition-all animate-fadeIn">
          <div className="flex items-center gap-2">
            <span className="p-1 rounded-full bg-emerald-500 text-slate-950">
              <Check className="w-3 h-3" />
            </span>
            <span>Successfully rotated! Today's questions and coding problems have been refreshed with a fresh curated set.</span>
          </div>
          <button
            onClick={() => setRefreshSuccess(false)}
            className="text-xs px-2 py-0.5 rounded hover:bg-emerald-500/20 text-emerald-800 dark:text-emerald-200"
          >
            Dismiss
          </button>
        </div>
